...
exports.run = async (client, message, args) => {
  if (
    !["751708415486853156", "751708415486853152", "751708415482658818"].some(
  ...
  let kadın = db.get(`ErkekUye.${message.author.id}`);
  let kayıtlar = db.fetch(`kayıtSayi.${message.author.id}`);

  const embed = new Discord.MessageEmbed()
    .setTitle(`<a:gokalp:773849287075037195> Kayıt İşlemi Tamamlandı`)
    .addField(`<a:neolduunubilimiyom:774213347670884352> Kayıt Eden:`, `<@${message.author.id}> Tarafından Kayıt Edildi`)
    .addField(`<a:neolduunubilimiyom:774213347670884352> Kayıt Edilen:`, `<@${stg.user.id}> Kayıt Oldu`)
    .addField(`<a:prizma:773849281429110784> Verilen Rol:`, `<@&${kayıtlı.id}> Rolleri Verildi`)
    .addField(`<a:sarok:773849277906157568> Alınan Rol:`, `<@&${kayıtsız.id}> Rolleri Alındı`)
    .addField(`<a:purplediamond:773849279478759445> Yeni İsmin:`, `\`${tag} ${isim} | ${yas}\` Olarak Güncellendi`)
    .addField(`<a:pin:773849289692151838> Yetkili Toplam:`, `\`${kayıtlar}\` Kayıtlara Sahip.`)
